...
class DATABASE:
  ...
  def add_dyspo_record(self, user_id, start_of_next_week, end_of_next_week):
    ...
    self.cursor.execute('SELECT ID FROM WEEK WHERE START_DAY = %s AND END_DAY = %s', (start_day, end_day))
    week_id = self.cursor.fetchone()

    if week_id is None:
      self.cursor.execute('INSERT INTO WEEK (START_DAY, END_DAY) VALUES (%s, %s)', (start_day, end_day))
  ...
  def add_form_sent_record(self, user_id, start_of_next_week, end_of_next_week):
    start_day = start_of_next_week.strftime('%Y.%m.%d')
    end_day = end_of_next_week.strftime('%Y.%m.%d')
    self.cursor.execute('INSERT INTO FORM_SENT (USER_ID, START_DAY, END_DAY) VALUES (%s, %s, %s)', (user_id, start_day, end_day))
    self.connection.commit()
  ...
  def is_form_sent_record_exists(self, user_id, start_of_next_week, end_of_next_week):
    ...
    self.cursor.execute('SELECT * FROM FORM_SENT WHERE USER_ID = %s AND START_DAY = %s AND END_DAY = %s', (user_id, start_day, end_day))
    result = self.cursor.fetchone()
    
    return result is not None


  def check_is_user_in_database(self, user_id):
    self.cursor.execute('SELECT * FROM USERS WHERE ID = %s', (user_id,))
    user_exists = self.cursor.fetchone()
    
    # If user does not exist, add them to the USERS table
    if not user_exists:
      self.cursor.execute('INSERT INTO USERS (ID) VALUES (%s)', (user_id,))
      self.connection.commit()
